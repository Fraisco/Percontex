<template>
  <div class="trasero-view">
    <h1 class="titulo-principal">🔨 Gestión de Procesos - Trasero</h1>

    <!-- Componente de múltiples órdenes -->
    <MultiOrdenManager proceso="Trasero" @orden-seleccionada="manejarOrdenSeleccionada"
      @ordenes-actualizadas="manejarOrdenesActualizadas" ref="multiOrdenManager">
      <template #default="{ orden, actualizar }">
        <div v-if="orden" class="proceso-trasero-container">
          ...
          <div class="agregar-proceso">
            <div class="input-grupo">
              ...
              <input type="text" id="nuevoProcesoNombre" v-model="nuevoProcesoInput" @keyup.enter="agregarNuevoProceso"
                placeholder="Ej: Proceso Extra Trasero" class="input-nuevo-proceso" />
            </div>
          ...
          <section class="gestor-subprocesos">
            ...
            <h3 class="titulo-seccion">📊 Seguimiento de Trasero</h3>
            <SubprocesoManager :orden="orden" :subprocesos="procesosTrasero" namespace="trasero" />
          </section>
                    ...
                    <tr :class="getClassesFila(estado, color, total, colorIndex)">
                      ...
                      <td class="celda-accion">
                        ...
                        <div v-if="procesoEnEdicion.procesoId === procesoId && procesoEnEdicion.color === color"
                          ...
                          <div v-if="procesoId === 'trasero_presillar'" class="unidad-medida">
                            <span>por</span>
          ...
          <GuardarCancelarDialog :orden-actual="orden" proceso-nombre="Trasero" :cambios-realizados="cambiosRealizados"
          ...
          <section class="gestor-subprocesos">
            <h3 class="titulo-seccion">Subprocesos - Trasero</h3>
            <SubprocesoManager :orden="orden" :subprocesos="procesosTrasero" namespace="trasero" />
          </section>
        </div>
      </template>
...
// inicializamos el store dentro de setup
...
const plantaUpdates = usePlantaRealtimeUpdates('trasero')
...
// Procesos predefinidos para Trasero
...
// Subprocesos dinámicos para navegación de planta (Trasero)
...
function guardarSubprocesosNav() {
  const personalizados = procesosTrasero.value.filter(p => p.custom === true)
  localStorage.setItem('subprocesosNavTrasero', JSON.stringify(personalizados))
}
function cargarSubprocesosNav() {
  const raw = localStorage.getItem('subprocesosNavTrasero')
  if (!raw) return
  try {
    const list = JSON.parse(raw)
    list.forEach(p => {
      if (!procesosTrasero.value.some(x => x.id === p.id)) {
        procesosTrasero.value.push({ id: p.id, nombre: p.nombre, custom: true })
      }
    })
  } catch { }
}
function agregarSubproceso() {
  ...
  if (procesosTrasero.value.some(p => p.id === id || p.nombre.toLowerCase() === nombre.toLowerCase())) return
  procesosTrasero.value.push({ id, nombre, custom: true })
...
// Mapeo de colores (igual que en delantero)
...
const todosLosProcesos = computed(() => {
  const all = [...procesosTrasero.value]
...
// Funciones principales (similares a delantero pero ajustadas para trasero)
    ...
    detallesOrden.forEach((detalle) => {
      ...
      if (cantidadTotalDetalle > 0) {
        todosLosProcesos.value.forEach((proceso) => {
          ...
          // Para 'trasero_presillar', cada prenda tiene 4 presillas
          if (proceso.id === 'trasero_presillar') {
            totalesOrdenActual[proceso.id][detalle.color] += cantidadTotalDetalle * 4
          } else {
...
function confirmarEntrada() {
  ...
  if (procesoId === 'trasero_presillar' &&
    (isNaN(unidadMedidaSeleccionada.value) || unidadMedidaSeleccionada.value <= 0)) {
    alert('Para Trasero Presillar, la unidad de medida debe ser un número positivo.')
    return
  }
...
function agregarNuevoProceso() {
  ...
  const newId = `custom_trasero_${Date.now()}`
...
function guardarEstadoLocal(ordenId) {
  ...
  localStorage.setItem(`procesosTraseroEstado_${ordenId}`, JSON.stringify(raw))
}

function cargarEstadoLocal(ordenId) {
  const raw = JSON.parse(localStorage.getItem(`procesosTraseroEstado_${ordenId}`) || '{}')
  return raw
}

function guardarProcesosPersonalizados() {
  localStorage.setItem('procesosPersonalizadosTrasero', JSON.stringify(procesosPersonalizados.value))
}

function cargarProcesosPersonalizados() {
  const stored = localStorage.getItem('procesosPersonalizadosTrasero')
  if (stored) {
...
function aplicarFiltroHorario() {
  ...
  actualizarRegistrosFiltradosPorHora()
  console.log(`[Trasero] Filtro horario aplicado: ${filtroHorario.desde} - ${filtroHorario.hasta}`)
}
...
function limpiarFiltroHorario() {
  ...
  console.log('[Trasero] Filtro horario limpiado')
}
...
function actualizarRegistrosFiltradosPorHora() {
  ...
  console.log(`[Trasero] Registros filtrados por hora: ${registrosFiltrados.length}`)
}
...
const handleVistaCambiando = (event) => {
  if (event.detail.vistaAnterior === 'Trasero') {
    console.log('[Trasero] Limpiando estado al cambiar de vista')
    cancelarEdicion()
    filtroProceso.value = ''
    filtroEstado.value = ''
    nuevoProcesoInput.value = ''
...
onMounted(() => {
  ...
  const handlePlantaUpdate = (event) => {
    if (event.detail.procesoTipo === 'trasero') {
      console.log('[Trasero] Actualización de proceso recibida:', event.detail)
      nextTick(() => {
        if (multiOrdenManager.value) {
          multiOrdenManager.value.actualizarEstado()
        }
  ...
  const handleRealtimeUpdate = (event) => {
    console.log('[Trasero] Actualización en tiempo real:', event.detail)
    actualizarCambiosRealizados()
  }

  const handleCrossBrowserSync = (event) => {
    if (event.detail.tipo === 'proceso_actualizado' && event.detail.datos.proceso === 'trasero') {
      console.log('[Trasero] Sincronización entre ventanas:', event.detail)
      setTimeout(() => {
        if (ordenActual.value?.id) {
          inicializarProcesosParaOrden(ordenActual.value.id, ordenActual.value.detalles)
        }
