...
function cargarDetallesCorte() {
  ...
  ordenesSeleccionadas.value.forEach((orden) => {
    ...
    // Elegir fuente: unidadesNumeradas (preferida) o unidadesCortadas
    let unidades = [];
    if (
      Array.isArray(orden?.estadoCorte?.unidadesNumeradas) &&
      orden.estadoCorte.unidadesNumeradas.length > 0
    ) {
      unidades = orden.estadoCorte.unidadesNumeradas;
    } else if (Array.isArray(orden?.estadoCorte?.unidadesCortadas)) {
      unidades = orden.estadoCorte.unidadesCortadas;
    }
    ...
    unidades.forEach((u) => {
      ...
      // Mapear nombres alternativos de campos
      const total = Number(
        u.total_cantidad ?? u.cantidad_total ?? u.total ?? u.cantidadTotal ?? 0
      );

      const cortadas = Number(
        u.total_numeradas ?? // API posible 1
        u.numeradas ?? // API posible 2
        u.cortadas ?? // API posible 3 (editor rápido)
        u.cantidadCorta ?? // Nombre que mencionaste
        u.cantidad_cortada ?? // Variante snake_case
        0
      );

      let faltante =
        u.unidades_faltantes ?? u.faltante ?? u.pendientes ?? u.cantidad_pendiente;

      // Convertir faltante a número si existe; si no, calcularlo
      faltante = Number.isFinite(Number(faltante))
        ? Number(faltante)
        : Math.max(0, total - cortadas);
      ...
      // Acumular (permitir entradas aunque total sea 0 si hay cortadas/faltante)
      detallesPorColor[color].cantidadTotal += Number(total) || 0;
      detallesPorColor[color].cantidadCortada += Number(cortadas) || 0;
      detallesPorColor[color].Faltante += Number(faltante) || 0;
    });

    // Guardar como arreglo por orden
  ...
  for (const orden of ordenesSeleccionadas.value) {
    ...
    if (detalles) {
      const unidadesCortadasParaGuardar = detalles.map((item) => ({
        ...
        total_numeradas: item.cantidadCortada,
      }));
      try {
        // Llamar función SQL directamente
        const { data, error } = await supabase.rpc("actualizar_tela_numerada", {
        ...
        console.log("✅ Unidades cortadas actualizadas:", data);
        alert("Tela numerada actualizada");
      } catch (error) {
...
function iniciarProcesoConSeleccionadas() {
  if (ordenesSeleccionadas.value.length > 0) {
    if (
      ...
      route.push({ name: "CortarMultiple" });
    } else {
...
function cargarDatosCorteRapida(orden) {
  const detalles = [];
  const unidades = orden?.estadoCorte?.unidadesCortadas || [];
  ...
  for (const u of unidades) {
    ...
    detalles.push({
      ...
      cantidadCortada: Number(u.total_numeradas ?? 0),
...
<template>
  <div class="proceso-corte-multiple-horizontal">
    <h2 class="titulo-seccion">Proceso de Numeración</h2>
    ...
    <div v-if="ordenesSeleccionadas.length > 0">
      <div class="resumen-corte">
        <h3>
          {{
            modoTrabajo === "individual"
              ? "Resumen de Numeracion para Orden Seleccionada"
              : "Resumen de Numeracion para Órdenes Seleccionadas"
          }}
        </h3>
        <p><strong>Progreso General:</strong> {{ progresoCorte.toFixed(2) }}%</p>
        <p><strong>Total Pedido:</strong> {{ totalPedidoOrden }} unidades</p>
        <p><strong>Total Numerada:</strong> {{ totalCortadoOrden }} unidades</p>
      ...
      <div class="campos-tiempo-unidades">
        <h4 class="subtitulo-tiempo">
          Control de Tiempo y Unidades - Proceso de Numeración
        </h4>
      ...
      <div class="botones-principales">
        <button @click="guardarEstadoCorte" class="boton-guardar">
          {{
            modoTrabajo === "individual"
              ? "Guardar Estado de Numeración"
              : "Guardar Estado de Numeración para Seleccionadas"
          }}
        </button>
    ...
    <div v-else class="mensaje-info">
      No se han seleccionado órdenes para el proceso de numeracion.
    </div>
  </div>
  ...
  <div v-if="editorRapidoActivo" class="editor-rapido-overlay">
    <div class="editor-rapido-modal">
      <div class="editor-rapido-header">
        <h3>Editor Rápido - Proceso de Numeración</h3>
      ...
      <div class="editor-rapido-content">
        ...
        <div v-if="ordenRapida && !cargandoOrdenRapida" class="datos-orden-rapida">
          ...
          <div v-if="detallesCorteTodasOrdenes.length > 0" class="tabla-corte-rapida">
            <h5>Cantidades Numeradas por Color:</h5>
        ...
        <div v-else class="sin-orden-seleccionada">
          <p>Busca y selecciona una orden para editar sus datos de numeracion.</p>
        </div>
      </div>
    </div>
  </div>
