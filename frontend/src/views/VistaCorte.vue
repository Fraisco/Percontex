...
function cargarDetallesCorte() {
  ...
  ordenesSeleccionadas.value.forEach((orden) => {
    ...
    if (
      orden?.estadoCorte?.unidadesCortadas &&
      orden.estadoCorte.unidadesCortadas.length > 0
    ) {
      orden.estadoCorte.unidadesCortadas.forEach((unidad) => {
  ...
  for (const orden of ordenesSeleccionadas.value) {
    ...
    if (detalles) {
      const unidadesCortadasParaGuardar = detalles.map((item) => ({
        ...
        total_cortadas: item.cantidadCortada,
      }));
      try {
        ...
        alert("Tela cortada actualizada");
      } catch (error) {
...
function cargarDatosCorteRapida(orden) {
  ...
  for (const u of unidades) {
    ...
    detalles.push({
      ...
      cantidadCortada: Number(u.total_cortadas ?? 0),
...
<template>
  <div class="proceso-corte-multiple-horizontal">
    <h2 class="titulo-seccion">Proceso de Corte</h2>
    ...
    <div v-if="ordenesSeleccionadas.length > 0">
      ...
      <div class="campos-tiempo-unidades">
        <h4 class="subtitulo-tiempo">Control de Tiempo y Unidades - Proceso de Corte</h4>
  ...
  <div v-if="editorRapidoActivo" class="editor-rapido-overlay">
    <div class="editor-rapido-modal">
      <div class="editor-rapido-header">
        <h3>Editor Rápido - Proceso de Corte</h3>
      ...
      <div class="editor-rapido-content">
        ...
        <div v-else class="sin-orden-seleccionada">
          <p>Busca y selecciona una orden para editar sus datos de corte.</p>
        </div>
      </div>
    </div>
  </div>
